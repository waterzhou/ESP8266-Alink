		...
		if (value_str) {
			...
			case 0:
				if (virtual_device.power != value) {
					...
					ESP_DBG(("power value=%d", value));
					if (value == 1)
						user_esp_set_on_power();
					else if (value == 0)
						user_esp_set_off_power();	
				}
				break;
			case 1:
				if (virtual_device.temp_value != value) {
					virtual_device.temp_value = value;
					ESP_DBG(("temp value=%d", value));
					light_set_aim(100,100,value,virtual_device.light_value,0,1000);	
				}
				break;
			case 2:
				if (virtual_device.light_value != value) {
					virtual_device.light_value = value;
					light_set_aim(100,100,virtual_device.temp_value,value,0,1000);	
					ESP_DBG(("light value=%d", value));
				}
				break;
			case 3:
				if (virtual_device.time_delay != value) {
					virtual_device.time_delay = value;
					ESP_DBG(("time_delay value=%d", value));
				}
				break;
			case 4:
